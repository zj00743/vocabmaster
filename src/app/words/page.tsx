...
function WordsPageInner() {
  ...
  const browseCategoryRaw = searchParams.get("category");
  const browseCategory =
    browseCategoryRaw && browseCategoryRaw.trim() !== ""
      ? browseCategoryRaw.trim()
      : null;
  /** Corpus browse: `/words?category=foo` · My Words slice: `/words?category=foo&in_my_words=1`. */
  const inMyWordsFromUrl =
    searchParams.get("in_my_words") === "1" ||
    searchParams.get("in_my_words") === "true";
  const browseCorpusInCategory =
    browseCategory !== null && !inMyWordsFromUrl;
  ...
  useEffect(() => {
    setSortBy(browseCorpusInCategory ? "frequency" : "added");
  }, [browseCorpusInCategory]);
  const [myWordCategories, setMyWordCategories] = useState<
  ...
  useEffect(() => {
    if (browseCategory) setCategoryFilter(browseCategory);
  }, [browseCategory]);

  useEffect(() => {
    if (browseCorpusInCategory) return;
    async function loadCategories() {
      try {
        const res = await fetch("/api/categories?in_my_words=1");
        if (res.ok) {
          setMyWordCategories(await res.json());
        }
      } catch {
        // ignore
      }
  ...
  }, [browseCorpusInCategory, filterParamsForBulk]);
  ...
  const canBulkSelect = !browseCorpusInCategory;
  ...
  return (
    <AppShell>
      <div className="px-4 py-6 md:px-8 md:py-8 max-w-2xl mx-auto w-full space-y-4 pb-28">
          ...
          <div>
            ...
            {!browseCorpusInCategory && (
              <p className="text-xs text-muted-foreground mt-1">
                {browseCategory
                  ? `Only words you've saved in My Words labeled “${browseCategory}”.`
                  : "Words you have added to your book (not the full dictionary)."}
              </p>
            )}
            {browseCorpusInCategory && (
              <p className="text-xs text-muted-foreground mt-1">
                Full word bank for this topic (includes words you have not
                added to My Words).
              </p>
            )}
          </div>
      ...
      {selectMode && canBulkSelect && (
        <div
          className="fixed inset-x-0 bottom-14 md:bottom-0 z-40 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 pb-[env(safe-area-inset-bottom)]"
          role="region"
          aria-label="Bulk selection actions"
        >
          <div className="mx-auto max-w-2xl px-4 py-2.5 md:px-8 flex flex-wrap items-center gap-2">
